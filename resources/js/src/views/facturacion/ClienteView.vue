...
const importClientes = (rows: Record<string, unknown>[]) =>
    clienteVentaService.import(rows);
    ...
    >
        <template #actions>
            <Button
                v-if="isSuperadmin"
                variant="outline"
                class="border-[#A8D2B7] text-[#126B3B]"
        ...
        <div class="grid gap-4 lg:grid-cols-[1fr_360px]">
            <section class="company-panel overflow-hidden">
                <div
                    class="grid gap-3 border-b border-[#dfe7e2] p-4 md:grid-cols-[1fr_160px_auto]"
                >
                    <div class="relative">
                        <Search
                ...
                <div
                    class="grid gap-3 border-b border-[#dfe7e2] p-4 sm:grid-cols-3"
                >
                    <div
                        class="rounded-lg border border-[#dfe7e2] bg-white p-3"
                    >
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-[#dfe7e2] text-sm">
                        <thead
                            class="bg-[#f6faf7] text-left text-xs font-semibold text-[#536158] uppercase"
                        >
                            <tr>
                        ...
                        <tbody class="divide-y divide-[#edf2ef] bg-white">
                            <tr v-if="loading">
                                <td
                                    colspan="5"
                                    class="px-4 py-8 text-center text-[#65736b]"
                                >
                                    Cargando clientes...
                                </td>
                            </tr>
                            ...
                            >
                                <td class="px-4 py-3">
                                    ...
                                    <p class="text-xs text-[#65736b]">
                                        {{ item.codigo }}
                                    </p>
                                </td>
                                <td class="px-4 py-3">
                                    <p class="font-mono text-[#101713]">
                                        {{ item.nit_ci
                                        }}{{
                                            item.complemento
                                                ? `-${item.complemento}`
            ...
            <aside class="company-panel p-4">
                <div class="mb-4 flex items-center gap-3">
                    <div
                        class="flex size-10 items-center justify-center rounded-lg bg-[#eaf7ef] text-[#168447]"
                    >
                ...
                <form class="space-y-4" @submit.prevent="save">
                    ...
                    <div class="space-y-2">
                        <Label for="tipo_documento_identidad"
                        ...
                        <InputError
                            :message="errors.tipo_documento_identidad?.[0]"
                        />
                    </div>
                    <div class="grid gap-3 sm:grid-cols-[1fr_110px]">
                        ...
                        <div
                            v-if="String(form.tipo_documento_identidad) === '1'"
                            class="space-y-2"
                        >
                    ...
                    <p
                        v-if="notice"
                        class="rounded-lg border border-[#dfe7e2] bg-[#f6faf7] px-3 py-2 text-sm text-[#536158]"
                    >
                        {{ notice }}
                    </p>
                    <div class="flex gap-2">
                        <Button
                            type="submit"
                            class="flex-1 bg-[#168447] text-white hover:bg-[#126B3B]"
                            :disabled="saving"
                        >
        ...
        <ImportExcelDialog
            ...
            description="Carga clientes desde Excel. Si ya existe el mismo documento y tipo, se actualiza."
            :columns="[
                'razon_social',
                'nit_ci',
                'tipo_documento_identidad',
