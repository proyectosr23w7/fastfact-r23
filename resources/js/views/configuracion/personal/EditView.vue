...
import type { BreadcrumbItem, Personal, Puesto, SharedData } from '@/types';
...
const form = useForm({
    nombre_completo: props.personalItem.nombre_completo,
    telefono: props.personalItem.telefono ?? '',
    numero_documento: props.personalItem.numero_documento,
    fecha_ingreso: props.personalItem.fecha_ingreso ?? '',
    puesto_id: props.personalItem.puesto_id ? String(props.personalItem.puesto_id) : '',
    activo: props.personalItem.activo,
});
...
const submit = () => {
    ...
    })).put(`/configuracion/personal/${props.personalItem.id}`);
};
</script>

<template>
    <Head :title="`Editar ${props.personalItem.nombre_completo}`" />
    <ModulePageLayout
        title="Editar personal"
        :breadcrumbs="breadcrumbs"
        description="Ajusta fichas del equipo manteniendo una base ordenada para reportes, permisos y sucursales."
    >
        <EntityFormCard
            :title="`Editar ${props.personalItem.nombre_completo}`"
            description="Actualiza datos del colaborador con una estructura lista para futuras ampliaciones del sistema."
        >
            <form class="space-y-6" @submit.prevent="submit">
                        ...
                        <label class="company-switch">
                            ...
                            <span class="space-y-1">
                                ...
                                <span class="block text-sm text-muted-foreground">
                                    Permite conservar al colaborador disponible para operaciones y asignaciones.
                                </span>
                            </span>
                        </label>
                ...
                <div class="flex flex-wrap items-center justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing" class="company-action-primary">
                        {{ form.processing ? 'Actualizando...' : 'Actualizar personal' }}
                    </Button>
                </div>
            </form>
        </EntityFormCard>
